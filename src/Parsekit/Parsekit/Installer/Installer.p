...
#   TODO warning. What if root is another package ?
    ...
    $loaderName[loader^math:sha1[^math:random(26082016)]]
    $loaderName[^loaderName.mid(0;14)]
    $string[#Do not edit this file. It is autogenerated by parsekit.
^@auto[]
^^use[parsekit/ClassLoader.p^;^$.replace(true)]
^^if(!def ^$MAIN:parsekitClassLoaders){
    ^$MAIN:parsekitClassLoaders[^^hash::create[]]
    ^$MAIN:parsekitAutoloadOrigin[^$MAIN:autouse]
}
...
^self.autoloadData.files.foreach[key;value]{^^${loaderName}.addFile[$value]}[^#0A]
^$MAIN:parsekitClassLoaders.[^^MAIN:parsekitClassLoaders._count[]][^$${loaderName}]
^^${loaderName}.register[]
^###
...
^@autouse^[className^]
    ...
    ^^if(!^$found){
        ^^try{
            ...
            ^^if(^$MAIN:parsekitAutoloadOrigin){
                ^$exception.handled(true)
                ^^MAIN:parsekitAutloadOrigin[^$className]
            }
        }
    }
^###
