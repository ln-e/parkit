...
^@auto[]
^^use[parsekit/ClassLoader.p]
^^if(!def ^$MAIN:parsekitClassLoaders){^$MAIN:parsekitClassLoaders[^^hash::create[]]}
^$loader[^^Parsekit/Parsekit/ClassLoader::create[]]
^self.autoloadData.classes.foreach[key;value]{^^loader.addClass[$key^;$value]}[^#0A]
^self.autoloadData.namespaces.foreach[key;value]{^^loader.addNamespace[$key^;$value]}[^#0A]
^self.autoloadData.classpath.foreach[i;val]{^^loader.addClasspath[$val]}[^#0A]
^self.autoloadData.files.foreach[key;value]{^^loader.addFile[$value]}[^#0A]
^$MAIN:parsekitClassLoaders.[^^MAIN:parsekitClassLoaders._count[]][^$loader]
^^loader.register[]
^###
...
#   For namespace we pre-search classes and build map.
    ^if($autoload.namespace is hash){
        ^autoload.namespace.foreach[namespacePrefix;path]{
            $path[^taint[as-is][^path.trim[left;/]]]
            ^if($addMap){
                $self.autoloadData.namespaces.$namespacePrefix[${docRoot}$path]
            }
            ...
            ^files.foreach[key;value]{
                ...
                ^if(^file.text.match[^@class\n$className][giUn] > 0){
                    ...
                    $self.autoloadData.classes.[$className][${docRoot}$value]
                }
            }
        }
    }
